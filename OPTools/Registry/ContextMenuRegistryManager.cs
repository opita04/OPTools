...
public class ContextMenuRegistryManager
{
    ...
    private ContextMenuEntry? GetEntryInfo(string shellPath, string entryName, string menuType)
    {
        try
        {
            ...
            using (var entryKey = _rootKey.OpenSubKey(entryKeyPath, false))
            {
                ...
                string? iconPath = entryKey.GetValue("Icon") as string;
    ...
    public (bool Success, string Message) AddEntry(string appPath, string menuName, List<string> menuTypes, string? iconPath = null)
    {
        // Validate app path
            ...
            try
            {
                ...
                using (var entryKey = _rootKey.CreateSubKey(entryKeyPath, true))
                {
                    if (entryKey != null)
                    {
                        ...
                        {
                            entryKey.SetValue("Icon", iconPath);
                        }
    ...
    public (bool Success, string Message) UpdateEntry(string registryPath, string newDisplayName, string newAppPath, string menuType, string? newIconPath = null)
    {
        try
        {
            ...
            using (var entryKey = _rootKey.OpenSubKey(registryPath, true))
            {
                if (entryKey == null)
                {
                    ...
                    if (!string.IsNullOrEmpty(newIconPath))
                    {
                        ...
                        {
                            entryKey.SetValue("Icon", newIconPath);
                        }
                        else
                        {
                            // Remove icon value if file doesn't exist
                            entryKey.DeleteValue("Icon");
                        }
                    }
                    else
                    {
                        // Remove icon value if no icon specified
                        try
                        {
                            entryKey.DeleteValue("Icon");
                        }
                        catch { /* Icon value didn't exist */ }
                    }
    ...
    /// </summary>
    public string ExtractExecutablePath(string command)
    {
